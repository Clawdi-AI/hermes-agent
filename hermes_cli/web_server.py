...
# web UI picks it up without any external coordination. The desktop shell
# can mint and pass ``HERMES_DASHBOARD_SESSION_TOKEN`` so its main process
# can authenticate API calls on the user's behalf.
#
...
# value both sides share. Falls back to the desktop token and then the
# random default when unset, preserving stand-alone CLI behavior.
# ---------------------------------------------------------------------------
_SESSION_TOKEN = (
    os.environ.get("HERMES_SESSION_TOKEN")
